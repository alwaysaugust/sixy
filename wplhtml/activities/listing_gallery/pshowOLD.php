...
<div class="wpl_gallery_container" id="wpl_gallery_container<?php echo $this->property_id; ?>" style="max-height: <?php echo $this->image_height; ?>px;width: <?php echo $this->image_width; ?>px">
       ...
       // $i = 0;
        ...
        foreach($this->gallery as $image)
        {
           $image_url = $image['url'];
            ...
            if($this->lazyload == 1)
            {
                echo '<li><img src="" data-src="' . $image_url . '" ' . $this->itemprop_image . ' class="lazyimg wpl_gallery_image ' . $this->image_class . '" id="wpl_gallery_image' . $image['raw']['id'] . '" width="' . $this->image_width . '" height="' . $this->image_height . '" alt="' . $image_alt . '" style="max-height:' . $this->image_height . 'px" /></li>';
 ...
 foreach ($exploded as $value) {
  echo "$value <br>";
                ...
                echo '<li><img src="' . $image_url . '"/><img src="' . $image_url . '"/><img src="' . $image_url . '"/>'.  "<script>console.log('Debug Objects: " . $image_url . "' );</script>" .'</li>';
                $pager_box .= '<a data-slide-index="' . 1 . '" href=""><img src="' . $image_thumbnail_url . '" width="100" height="80" style="width: 100px; height: 80px;" itemprop="image" alt="' . $image_alt . '" /></a>';
    ...
    <div class="wpl-slider-bx-pager-wp" id="bx-pager-<?php echo $this->activity_id; ?>">
        <?php echo '<div class="wpl-slider-bx-img-count" id="img-count-'.$this->activity_id.'">'.count($this->gallery).'</div>'.$pager_box; ?>
    </div>
    <?php 
	}
