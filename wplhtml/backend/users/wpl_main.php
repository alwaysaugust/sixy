...
class wpl_users_controller extends wpl_controller
{
	...
	public function user_manager()
	{
        ...
        if(trim($this->parent)) $where_query = " AND `parent`='".$this->parent."'";
	...
	public function profile($instance = array())
	{
		...
		{
			/** import message tpl **/
			$this->message = __("You don't have access to this menu!", 'real-estate-listing-realtyna-wpl');
		...
		{
			/** import message tpl **/
			$this->message = __("You're not an agent/owner so you cannot access this profile.", 'real-estate-listing-realtyna-wpl');
			return parent::render($this->tpl_path, 'message');
		}
