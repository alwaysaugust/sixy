...
global $wpdb;
$q = "SELECT * FROM {$wpdb->base_prefix}wpl_users WHERE id > 0 AND blog_id = '".get_current_blog_id()."' LIMIT 1";
$agent = $wpdb->get_row($q);
$user_ids = array($agent->id);
...
foreach($user_ids as $user_id)
{
    ...
    $file_headers = @get_headers($profile_image);
    if(!$file_headers || $file_headers[0] == 'HTTP/1.1 404 Not Found') {
        $profile_image = preg_replace('/\/WPL\d+\//', '/WPL/', $profile_image);
    }
    
    if (!file_exists($profile_path)) {
        $profile_path = preg_replace('/\/WPL\d+\//', '/WPL/', $profile_path);
    }
