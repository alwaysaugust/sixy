...
function wpl_send_contact<?php echo $this->activity_id; ?>(property_id)
{
	...
	ajax.success(function(data)
	{
		if(data.success === 1)
		{
            ...
            <?php if(wpl_global::check_addon('listhub') and $this->settings['listhub_tracking_status'] == '1'): ?>
            	lh('submit', 'AGENT_EMAIL_SENT', {lkey:'<?php echo $wpl_properties['current']['raw']['listing_key']; ?>'});
            <?php endif; ?>
		}
    ...
    {
        wpl_show_messages("<?php echo addslashes(__('Error Occurred!', 'real-estate-listing-realtyna-wpl')); ?>", '#wpl_contact_message<?php echo $this->activity_id; ?>_'+property_id, 'wpl_red_msg');
    });
