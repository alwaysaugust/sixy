...
function wpl_send_to_friend_get_form(id)
{
    ...
    {
        wplj("<?php echo $this->lightbox_container; ?>").html(html);
        if(wplj("<?php echo $this->lightbox_container; ?>").find('.g-recaptcha').length > 0) {
            grecaptcha.render(document.getElementById(wplj("<?php echo $this->lightbox_container; ?>").find('.g-recaptcha').first().attr('id')));
        }
    });
