...
/* Flare Rush */
$flare_path = ABSPATH . '../rush/vendor/autoload.php';
if(file_exists($flare_path)) include_once $flare_path;
...
class wpl_property_listing_controller extends wpl_controller
{
    ...
    private function advanced_locationtextsearch_autocomplete($term)
    {
        ...
        if(class_exists('Flare\Rush\Suggestion'))
        {
            ...
            {
                $output[] = array('label' => $match['phrase'], 'title' => $labels[$match['type']], 'column' => $match['type'], 'value' => $match['phrase']);
            }
        }
        else
        {
