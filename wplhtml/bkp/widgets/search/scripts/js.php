...
// jQuery(document).ready(function(){
//     jQuery('.chosen-results .active-result').each(function(e,i){
...
//     });
    ...
    wplj(document).on('click', '.wpl_search_from_box #more_search_option<?php echo $this->widget_id; ?>', function()
    {
        ...
        {
            clearContent: false,
            title: "<?php echo __('Advanced Search', 'real-estate-listing-realtyna-wpl'); ?>",
            cssClasses: {wrap : 'wpl-frontend-lightbox-wp wpl-frontend-search-lightbox-wp'},
            callbacks:
            {
                beforeOpen: function()
                {
                ...
                afterClose: function()
                {
                    ...
                    wpl_render_checkbox_select_<?php echo $this->widget_id; ?>();
                    wplj('.wpl-search-mobile .wpl_search_from_box_top .wpl_search_field_textsearch').on('click', function () {
                        wplj(this).parent().find('.more_search_option').trigger('click');
                    });
                }
            }
        });
    ...
    <?php else: ?>
        wplj('.wpl_search_from_box #more_search_option<?php echo $this->widget_id; ?>').on('click', function () {
            ...
            if (wplj(this).hasClass('active')) {
                ...
                wplj(this).text("<?php echo __('Advanced', 'real-estate-listing-realtyna-wpl'); ?>");
            }
            else {
    ...
    wpl_set_total_results_handler_<?php echo $this->widget_id; ?>();
    
    // First Run
    wpl_show_total_results_<?php echo $this->widget_id; ?>();
    <?php endif; ?>
});
...
function wpl_get_request_str_<?php echo $this->widget_id; ?>(force)
{
    if(typeof force == 'undefined') force = false;
    
    var ajax = force ? force : <?php echo (trim($this->ajax) ? '1' : '0'); ?>;
    request_str = '';
    ...
    wplj("#wpl_searchwidget_<?php echo $this->widget_id; ?> input[type='number']").each(function (index, element) {
        id = element.id;
        name = element.name;
        if (name.substring(0, 2) == 'sf') {
            if (wplj("#wpl_searchwidget_<?php echo $this->widget_id; ?> #" + id).closest('li').css('display') != 'none') {
    ...
    return 'widget_id=<?php echo $this->widget_id; ?>&kind=<?php echo $this->kind; ?>' + request_str;
}

/** main search function **/
function wpl_do_search_<?php echo $this->widget_id; ?>()
{
    request_str = wpl_get_request_str_<?php echo $this->widget_id; ?>();

    /** Create full url of search **/
    <?php
        $search_page = $this->get_target_page((isset($this->target_id) ? $this->target_id : NULL));

        // Apply Filters
        @extract(wpl_filters::apply('search_widget_search_page', array('search_page'=>$search_page, 'target_id'=>$this->target_id)));
    ?>
    search_page = '<?php echo $search_page; ?>';
...
function wpl_set_total_results_handler_<?php echo $this->widget_id; ?>()
{
	wplj("#wpl_search_form_<?php echo $this->widget_id; ?> input, #wpl_search_form_<?php echo $this->widget_id; ?> select, #wpl_search_form_<?php echo $this->widget_id; ?> textarea").off('change.wpl_total_results').on('change.wpl_total_results', function()
	{
		setTimeout("wpl_show_total_results_<?php echo $this->widget_id; ?>()", 300);
	});
...
var wpl_total_results_ongoing<?php echo $this->widget_id; ?> = false;
function wpl_show_total_results_<?php echo $this->widget_id; ?>()
{
    request_str = wpl_get_request_str_<?php echo $this->widget_id; ?>(true);
	...
	if(wpl_total_results_ongoing<?php echo $this->widget_id; ?>) return false;
	wpl_total_results_ongoing<?php echo $this->widget_id; ?> = true;
    ...
    wplj.ajax(
    {
        ...
        {
            wplj("#wpl_total_results<?php echo $this->widget_id; ?> span").text(data.total);
            wpl_total_results_ongoing<?php echo $this->widget_id; ?> = false;
        }
    });
}
...
(function($){
    ...
    $(function(){
        ...
        try{
            ...
            if(typeof $.fn.chosen != 'undefined') $("#wpl_search_form_<?php echo $this->widget_id; ?> select").chosen({ width: 'initial', search_contains:true, enable_split_word_search:false });
            else
        ...
        $(document).on('blur','#wpl_searchwidget_<?php echo $this->widget_id; ?> input[type="number"]', function(event)
        {
            element_id = event.target.id;
            value = $('#'+element_id).val();
            $('#'+element_id).attr('value', value);
...
function wpl_render_checkbox_select_<?php echo $this->widget_id; ?>()
{
    ...
    wplj('[id*="slider<?php echo $this->widget_id; ?>_range"]').each(function () {
        wplj(this).empty();
    });

    if(typeof wpl_slider_price_range<?php echo $this->widget_id; ?>_sale == 'function') {
        wpl_slider_price_range<?php echo $this->widget_id; ?>_sale();
    }
    if(typeof wpl_slider_price_range<?php echo $this->widget_id; ?>_rental == 'function') {
        wpl_slider_price_range<?php echo $this->widget_id; ?>_rental();
    }
    if(typeof wpl_slider_number_range<?php echo $this->widget_id; ?> == 'function') {
        wpl_slider_number_range<?php echo $this->widget_id; ?>();
    }
    if(typeof wpl_slider_area_range<?php echo $this->widget_id; ?> == 'function') {
        wpl_slider_area_range<?php echo $this->widget_id; ?>();
    }
}
...
function wpl_refresh_searchwidget_counter()
{
    wplj.ajax(
    {
        ...
        success: function(data)
        {
            wplj('#wpl-addon-save-searches-count<?php echo $this->widget_id; ?>').html(data.saved_searches);
            wplj('#wpl-widget-favorites-count<?php echo $this->widget_id; ?>').html(data.favorites);
        }
    });  
}
