...
$MapViewUrl = $_SERVER['REQUEST_URI'];
$MapViewUrlParts = explode('/', $MapViewUrl);
$FinalMapViewUrl = $MapViewUrlParts[2];
...
foreach($this->wpl_properties as $key=>$property)
{
	...
	<?php
	if ($FinalMapViewUrl!="mapview"){
			echo '<div class="wpl_prp_top">
				  <div class="wpl_prp_top_boxes front">';}?>
					<?php if ($FinalMapViewUrl!="mapview"){ wpl_activity::load_position('wpl_property_listing_image', array('wpl_properties'=>$this->wpl_properties)); }?>
			
					<?php
					if ($FinalMapViewUrl!="mapview"){
						echo '</div>
						<div class="wpl_prp_top_boxes back">
							<a';}?> <?php if ($FinalMapViewUrl!="mapview"){ echo $this->itemprop_url;?> id="prp_link_id_<?php echo $property['data']['id']; ?>" href="<?php echo $property['property_link']; ?>" class="view_detail"><?php echo __('More Details', 'real-estate-listing-realtyna-wpl'); }?>
					<?php
					if ($FinalMapViewUrl!="mapview"){
					echo	'</a>
						</div>
