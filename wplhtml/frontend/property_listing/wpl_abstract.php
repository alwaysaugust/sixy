...
abstract class wpl_property_listing_controller_abstract extends wpl_controller
{
    ...
    public function display($instance = array())
    {
        ...
        {
            if($this->wplraw and $this->method == 'get_markers') $wpl_property_ids[] = $property->id;
            else
            {
        ...
        {
            $query = "SELECT id, kind, listing, property_type, price, price_unit, googlemap_lt, googlemap_ln FROM `#__wpl_properties` WHERE `id` IN (" . implode(',', $wpl_property_ids) . ")";
            $raw_properties = wpl_db::select($query, 'loadAssocList');

            foreach($raw_properties as $raw_property) $wpl_properties[$raw_property['id']] = ['raw' => $raw_property];
        }

        // Define Current Index
        $wpl_properties['current'] = array();

        // Apply Filters (This filter must place after all process)
